...
export default function FormClient() {
  ...
  const Star = ({ n }) => (
    <button type="button" onClick={() => setRating(n)}
      style={{
        ...
        background: rating===n ? '#111':'#fff', color: rating===n ? '#fff':'#111', cursor: 'pointer'
      }}>
      {n}★
    </button>
  );
  ...
  const NpsBtn = ({ v }) => (
    <button type="button" onClick={() => setNps(v)}
      style={{
  ...
  return (
    <main style={{ maxWidth: 720, margin: '0 auto', padding: 20 }}>
      ...
      <Section title="Motivo principal (opcional):">
        <div style={{ display:'flex', gap: 8, flexWrap:'wrap' }}>
          {TAGS.map(t => <Chip key={t} active={tags.includes(t)} onClick={() => toggleTag(t)}>{t}</Chip>)}
        </div>
      </Section>

      <Section title="Tiempo de espera (opcional):">
        <div style={{ display:'flex', gap: 8, flexWrap:'wrap' }}>
          {WAIT.map(opt => <Chip key={opt} active={waitTime===opt} onClick={() => setWaitTime(opt)}>{opt}</Chip>)}
        </div>
      </Section>

      <Section title="Tipo de trámite (opcional):">
        <div style={{ display:'flex', gap: 8, flexWrap:'wrap' }}>
          {TRAMITE.map(opt => <Chip key={opt} active={tramite===opt} onClick={() => setTramite(opt)}>{opt}</Chip>)}
        </div>
      </Section>
      ...
      <Section title="¿Deseas que te contactemos?">
        ...
        {wantFollow && (
          <input value={contact} onChange={e=>setContact(e.target.value)} placeholder="Tu correo o teléfono"
